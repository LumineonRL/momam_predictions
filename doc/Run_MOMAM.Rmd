...
past_results_pivoted_metadata <- past_results_categorical %>%
  ...
  select(-category) # category is handled but the cumulative time function below.
```
...
# recent one.
data_train <- past_results_weighted[which(pivoted_data$MOMAM != 7), ]
data_test <- past_results_weighted[which(pivoted_data$MOMAM == 7), ]
```

Applies the pre-processing steps to our test and train data as defined above.
```{r apply_preprocessing}
data_processed <- prep(data_recipe, training = data_train, retain = TRUE) %>%
...
workflow_set <- workflow_set(
  preproc = list(simple = data_recipe),
  models = list(
...
```{r train}
set.seed(0323)
trained_workflow_set <- workflow_map(
  workflow_set,
  "fit_resamples",
